...
### 案例思考：  
- **历史学**：如果我们说某个王朝在100年间人口从1000万增加到1500万，那么这100年间的平均年人口增长数是多少？这就是一个平均变化率。  
- **文学研究**：一位作家早年作品平均每页使用10个形容词，晚年作品平均每页使用5个形容词。这描述了他写作风格在形容词使用上的平均变化。  
...
- 斜率为正，表示 y 随 x 增大而增大（上升的坡）。  
- 斜率为负，表示 y 随 x 增大而减小（下降的坡）。  
...
### 经济学中的边际概念：  
- **边际成本 (Marginal Cost)**：多生产一个单位产品所需要额外增加的成本。  
  例子：一个面包店已经烤了100个面包，再多烤1个面包（第101个）所额外增加的面粉、电力、人工等成本，就是这第101个面包的边际成本。  
- **边际收益 (Marginal Revenue)**：多销售一个单位产品所能额外增加的收入。  
  例子：书店卖出第50本书后，再多卖一本（第51本）所获得的额外收入。  
- **边际效用 (Marginal Utility)**：消费者多消费一个单位商品所能获得的额外满足感。  
  例子：你非常饿的时候吃第一个包子，感觉满足感爆棚（边际效用高）；吃到第五个包子时，可能已经很饱了，再多吃一个（第六个）带来的额外满足感就大大降低，甚至可能是负的（边际效用低或负）。这就是著名的“边际效用递减规律”。  
